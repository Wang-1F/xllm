...
ForwardInput RecPrefillOnlyBatchInputBuilder::build_rec_forward_input(
  ...
  std::vector<torch::Tensor> writeback_lens_i32;
  ...
  for (auto* sequence_group : sequence_groups_) {
    for (const auto& seq_ptr : sequence_group->sequences()) {
      ...
      const int32_t cacheable_len = cache_decision.cacheable_len;
      const int32_t writeback_len = cache_decision.writeback_len;
      ...
      if (cache_decision.high_value) {
        COUNTER_ADD(mtgr_high_value_matched_prefix_tokens_total,
                    matched_prefix);
      } else {
        COUNTER_ADD(mtgr_low_value_matched_prefix_tokens_total,
                    matched_prefix);
      }
      ...
      writeback_lens_i32.emplace_back(
          torch::tensor({writeback_len}, i32_options));
  ...
  mtgr_params.mtgr_writeback_lens_i32 =
      torch::cat(writeback_lens_i32).contiguous();
  ...
  MTGR_TRACE(2) << "[BATCH] mtgr_params segment_offsets="
                ...
                << " writeback_lens="
                << mtgr_params.mtgr_writeback_lens_i32.sizes();
