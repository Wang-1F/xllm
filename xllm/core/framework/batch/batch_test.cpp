...
TEST(BatchTest, MtgrRecPrefillBuilderTrimsEmbeddingAtPrefixBoundary) {
  ...
  EXPECT_TRUE(
      equal(mtgr_params->mtgr_segment_offsets_i32,
            std::vector<int32_t>{0, 2, 3, 4, 5}));
  EXPECT_TRUE(equal(mtgr_params->mtgr_segment_rules_i32,
                    std::vector<int32_t>{0, 1, 0, 2}));
  ...
  EXPECT_TRUE(equal(mtgr_params->mtgr_writeback_lens_i32,
                    std::vector<int32_t>{4}));
...
TEST(BatchTest, MtgrValueGatedLowValueDoesNotAllocateWritebackSlots) {
  ...
  FLAGS_mtgr_kv_cache_prefix_len_threshold = 10;
  ...
  EXPECT_TRUE(
      equal(forward_input.token_ids, std::vector<int32_t>{11, 12, 13, 14, 15}));
  EXPECT_EQ(sequence->kv_state().kv_cache_tokens_num(), 0);
  EXPECT_EQ(forward_input.input_params.new_cache_slots.numel(), 0);
  EXPECT_EQ(forward_input.input_params.block_tables.numel(), 0);
