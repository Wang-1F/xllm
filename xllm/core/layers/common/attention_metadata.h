...
// Batch-level cache for MTGR FlashInfer token-mask prefill metadata. The
// segment layout, cu-seqlens, packed custom mask and FlashInfer plan are
// identical for every decoder layer in the same forward pass, while K/V values
// remain layer-local and are still gathered per layer.
struct MTGRFlashinferTokenMaskCache {
  ...
  int64_t head_dim = 0;
  torch::ScalarType query_dtype = c10::ScalarType::Undefined;
  torch::ScalarType key_dtype = c10::ScalarType::Undefined;
  torch::ScalarType output_dtype = c10::ScalarType::Undefined;

  torch::Tensor q_cu_seq_lens_host;
  torch::Tensor kv_cu_seq_lens_host;
  torch::Tensor mask_indptr_host;
  torch::Tensor q_cu_seq_lens_device;
  torch::Tensor kv_cu_seq_lens_device;
  torch::Tensor mask_indptr_device;
...
struct AttentionMetadata {
  ...
  // for MTGR FlashInfer token-mask base path (shared by all decoder layers)
  std::shared_ptr<MTGRFlashinferTokenMaskCache>
      mtgr_flashinfer_token_mask_cache;
#endif
  ...
  // Shape: [batch_size]. Type: int32 device tensor. Logical token count that
  // may be written to persistent KV cache. 0 gates writeback off per request.
  torch::Tensor mtgr_writeback_lens_i32;
