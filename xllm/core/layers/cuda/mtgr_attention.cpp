...
MTGRAttentionImpl::forward(const AttentionMetadata& attn_metadata,
  ...
  MTGR_TRACE(2) << "[ATTN] mtgr_attention launch_shapes query_snd="
                ...
                << " writeback_lens="
                << attn_metadata.mtgr_writeback_lens_i32.sizes();
  ...
  if (cache_policy == MTGRCachePolicy::kFullSequence ||
      cache_policy == MTGRCachePolicy::kDynamicLengthAdmission) {
    ...
    kernel::cuda::mtgr_kv_cache_writeback_cuda(
        ...
        attn_metadata.mtgr_writeback_lens_i32,
        attn_metadata.block_table,
        key_cache,
        value_cache,
        attn_metadata.max_seq_len);
  } else if (cache_policy == MTGRCachePolicy::kPrefixOnly ||
             cache_policy == MTGRCachePolicy::kPrefixOnlyValueGated ||
             cache_policy == MTGRCachePolicy::kPrefixOnlyReuseHorizon) {
    MTGR_NVTX_RANGE(1, "MTGR/kernel/kv_prefix_writeback_call");
    MTGR_TRACE(1) << "[ATTN] kv_writeback=prefix_only_like";
  ...
  if (use_flashinfer_token_mask_base) {
    ...
    kernel::cuda::mtgr_flashinfer_token_mask_attention_cuda(
        ...
        scale_,
        output_snd,
        attn_metadata.mtgr_flashinfer_token_mask_cache);
  } else {
