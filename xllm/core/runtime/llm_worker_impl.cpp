...
std::optional<ForwardOutput> LLMWorkerImpl::step(const ForwardInput& input) {
  // LOG(INFO) << "input.input_params.batch_forward_type: " <<
...
void LLMWorkerImpl::update_for_next_step(
    ForwardInput& input,
    int32_t round,
  ...
  beam_shared_kv_expanded =
      beam_shared_kv_expanded.unsqueeze(1).expand({-1, beam_size, -1});
  ...
  shared_kv_len_offsets =
      shared_kv_len_offsets.unsqueeze(1).expand({-1, beam_size, -1});
  // LOG(INFO) << "shared_kv_len_offsets: " << shared_kv_len_offsets;
  ...
  auto shared_batch_offsets =
      torch::zeros({batch_size, beam_size, shared_kv_len}, paged_options);
  ...
  auto batch_beam_lens_offsets =
      kv_cu_seq_lens.slice(0, 0, -1).unsqueeze(1).expand({-1, shared_kv_len});
  batch_beam_lens_offsets =
      batch_beam_lens_offsets.unsqueeze(1).expand({-1, beam_size, -1});
  shared_batch_offsets = shared_batch_offsets + batch_beam_lens_offsets;
  // LOG(INFO) << "shared_batch_offsets: " << shared_batch_offsets;
  ...
  uint32_t unshared_begin_index = 0;
  ...
  // full_kv_indices; 计算 mask auto shared_mask = mask.unsqueeze(1).expand({-1,
  // beam_size, -1});
  ...
  // 过滤 indices
  auto shared_paged_kv_indices = shared_kv_indices;
  // LOG(INFO) << "shared_paged_kv_indices: " << shared_paged_kv_indices;
  auto unshared_paged_kv_indices =
      unshared_kv_indices.masked_select(unshared_mask);
  // LOG(INFO) << "unshared_paged_kv_indices: " << unshared_paged_kv_indices;
  auto shared_paged_kv_indptr_offsets =
      batch_shared_kv_lens.unsqueeze(1).expand({-1, beam_size});
  shared_paged_kv_indptr_offsets =
      shared_paged_kv_indptr_offsets.flatten().cumsum(0);
  shared_paged_kv_indptr_offsets = torch::cat(
      {torch::zeros({1}, paged_options), shared_paged_kv_indptr_offsets}, 0);
  // LOG(INFO) << "shared_paged_kv_indptr_offsets: " <<
  // shared_paged_kv_indptr_offsets;
  auto shared_paged_kv_indptr = shared_paged_kv_indptr_offsets;
  shared_paged_kv_indptr = shared_paged_kv_indptr.to(paged_options);

  // LOG(INFO) << "shared_paged_kv_indptr: " << shared_paged_kv_indptr;
  ...
  auto unshared_paged_kv_indptr =
      torch::full({batch_size * beam_size}, unshared_kv_len, paged_options);
  unshared_paged_kv_indptr = unshared_paged_kv_indptr.flatten().cumsum(0);

  unshared_paged_kv_indptr = torch::cat(
      {torch::zeros({1}, paged_options), unshared_paged_kv_indptr}, 0);
  unshared_paged_kv_indptr = unshared_paged_kv_indptr.to(paged_options);
  // LOG(INFO) << "unshared_paged_kv_indptr: " << unshared_paged_kv_indptr;
  // LOG(FATAL) << "t";
  ...
  // torch::cat expects all tensors to have the same dtype/device/options, so
  // ensure both tensors use paged_options. auto paged_kv_indptr = torch::cat(
  //     {torch::zeros({1}, paged_options), cumsum_result.to(paged_options)}, 0
  // );

  // 计算 paged_kv_last_page_len
  // auto shared_paged_kv_last_page_len = torch::ones({batch_size},
  // paged_options);
  auto paged_kv_last_page_len =
      torch::ones({batch_size * beam_size}, paged_options);

  // 设置到 input_params 中
  input.input_params.shared_paged_kv_indices = shared_paged_kv_indices;
  input.input_params.unshared_paged_kv_indices = unshared_paged_kv_indices;
  input.input_params.shared_paged_kv_indptr = shared_paged_kv_indptr;
  input.input_params.unshared_paged_kv_indptr = unshared_paged_kv_indptr;
  input.input_params.shared_paged_kv_last_page_len = paged_kv_last_page_len;
  input.input_params.unshared_paged_kv_last_page_len = paged_kv_last_page_len;
  ...
  //   auto dummy_query = torch::empty({batch_size * beam_size, num_heads,
  //   head_dim},
  ...
  //   unshared_k_cache_first = unshared_k_cache_first.view({-1, 1,
  //   num_kv_heads, head_dim}); input.input_params.plan_info =
...
std::optional<ForwardOutput> LLMWorkerImpl::step_multi_round(
  ...
  int64_t num_kv_heads =
      context_.get_model_args().n_kv_heads().value_or(num_heads);

  input.input_params.num_heads = num_heads;
  input.input_params.head_dim = head_dim;
  input.input_params.beam_width = input.beam_width;
  ...
  for (int32_t round = 0; round < total_rounds; ++round) {
    ...
    // LOG(INFO) << "logits.shape: " << logits.sizes(); //
    // [selected_token_idxes.dim(), 151936]
    ...
    if (sampling_params.selected_token_idxes.defined()) {
      ...
      // out_beam_count_prefix_sums_ptr,  # [B*BEAM_SIZE, 1] -
      // beam计数前缀和（未使用） out_sequence_ptr,    # [B, BEAM_SIZE,
      // total_rounds]
      // - 输出序列（写入）
      ...
      // LOG(INFO) << "out_beam_count_prefix_sums: " <<
      // out_beam_count_prefix_sums; LOG(INFO) << "out_seqgroup: " <<
      // out_seqgroup;
...
// LOG(INFO) << "out_seqgroup: " << out_seqgroup;
      ...
      if (round < total_rounds - 1) {
        ...
        if (round > 0) {
          ...
          // LOG(INFO) << "out_token_index: " << out_token_index;
          // LOG(INFO) << "out_beam_count_prefix_sums: " <<
          // out_beam_count_prefix_sums; LOG(INFO) << "round: " << round;
      ...
      if (round == total_rounds - 1) {
        ...
        // auto tmp_acc_logprob = acc_logprob.view({batch, beam_width});
        for (int i = 0; i < batch; i++) {
          for (int j = 0; j < beam_width; j++) {
            std::string str{};
            for (int k = 0; k < sequence_group.size(2); k++) {
              if (k > 0) {
                str += ", ";
              }
              str += std::to_string(sequence_group[i][j][k].item<int32_t>());
            }
            // str += ": ";
