...
DEFINE_string(mtgr_kv_cache_policy,
              "auto",
              "MTGR KV-cache policy. Supported values: auto, full_cache, "
              "full_sequence, prefix_only, prefix_only_value_gated, "
              "dynamic_length_admission, free_block_admission, "
              "full_cache_dynamic_length_admission, "
              "prefix_only_reuse_horizon, reuse_horizon, rha_writeback.");
DEFINE_string(mtgr_kv_cache_value_fn,
              "prefix_len",
              "MTGR KV-cache value function for value-gated policy. "
              "Currently only prefix_len is supported.");
DEFINE_int32(mtgr_kv_cache_prefix_len_threshold,
             0,
             "Prefix length threshold for MTGR prefix_only_value_gated policy. "
             "Requests with prefix_len below this threshold are not admitted "
             "into persistent prefix cache.");
DEFINE_double(mtgr_rha_pressure_free_block_ratio,
              0.10,
              "Free-block ratio at or below which MTGR reuse-horizon admission "
              "starts skipping cold low-reuse prefix-only writeback.");
DEFINE_int32(mtgr_rha_min_reuse_count,
             2,
             "Minimum observed prefix-family reuse count before MTGR "
             "reuse-horizon admission treats a cold request as reusable.");
DEFINE_int32(mtgr_rha_cold_prefix_len_threshold,
             2000,
             "Requests at or below this length may be skipped by MTGR "
             "reuse-horizon admission under cache pressure when no reuse "
             "evidence exists.");
DEFINE_int32(mtgr_rha_entity_hot_skip_min_seen,
             0,
             "If positive, entity-id backed MTGR reuse-horizon admission may "
             "skip short hot entities under pressure after this many sightings.");
DEFINE_uint64(mtgr_rha_state_max_entries,
              100000,
              "Maximum number of prefix-family states kept by MTGR "
              "reuse-horizon admission. 0 disables this cap.");
