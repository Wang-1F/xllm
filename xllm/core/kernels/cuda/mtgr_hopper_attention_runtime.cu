...
bool mtgr_token_mask_cache_matches(
    const xllm::layer::MTGRFlashinferTokenMaskCache& cache,
    const torch::Device& device,
    int64_t total_q,
    int64_t num_q_heads,
    int64_t num_kv_heads,
    ...
    torch::ScalarType key_dtype,
    torch::ScalarType output_dtype) {
  return cache.initialized && cache.total_q == total_q &&
         cache.num_q_heads == num_q_heads && cache.num_kv_heads == num_kv_heads &&
         cache.head_dim == head_dim && cache.query_dtype == query_dtype &&
         cache.key_dtype == key_dtype && cache.output_dtype == output_dtype &&
         cache.q_cu_seq_lens_host.defined() &&
         cache.kv_cu_seq_lens_host.defined() &&
         cache.mask_indptr_host.defined() &&
         cache.q_cu_seq_lens_device.defined() &&
         cache.kv_cu_seq_lens_device.defined() &&
         cache.mask_indptr_device.defined() && cache.packed_custom_mask.defined() &&
         cache.q_cu_seq_lens_device.device() == device &&
         cache.kv_cu_seq_lens_device.device() == device &&
         cache.mask_indptr_device.device() == device &&
         cache.packed_custom_mask.device() == device &&
         !cache.plan_uri.empty() && cache.plan_info.defined();
}

void mtgr_build_token_mask_cache(
    xllm::layer::MTGRFlashinferTokenMaskCache& cache,
    const torch::Device& device,
    const torch::Tensor& segment_offsets_i32,
  ...
  cache.total_q = total_q;
  cache.batch_size = host_meta.batch_size;
  cache.num_segments = host_meta.num_segments;
  cache.total_kv_len = host_meta.total_kv_len;
  cache.total_mask_bytes = host_meta.total_mask_bytes;
  cache.max_kv_len = host_meta.max_kv_len;
  cache.max_mask_bytes_per_request = host_meta.max_mask_bytes_per_request;
  cache.num_q_heads = num_q_heads;
  cache.num_kv_heads = num_kv_heads;
  cache.head_dim = head_dim;
  cache.query_dtype = query_dtype;
  cache.key_dtype = key_dtype;
  cache.output_dtype = output_dtype;

  cache.q_cu_seq_lens_host =
      mtgr_make_i32_cpu_tensor(host_meta.q_cu_seq_lens);
  cache.kv_cu_seq_lens_host =
      mtgr_make_i32_cpu_tensor(host_meta.kv_cu_seq_lens);
  cache.mask_indptr_host = mtgr_make_i32_cpu_tensor(host_meta.mask_indptr);
  cache.q_cu_seq_lens_device =
      cache.q_cu_seq_lens_host.to(device).contiguous();
  cache.kv_cu_seq_lens_device =
      cache.kv_cu_seq_lens_host.to(device).contiguous();
  cache.mask_indptr_device = cache.mask_indptr_host.to(device).contiguous();

  cache.packed_custom_mask =
      torch::empty({cache.total_mask_bytes},
                   torch::TensorOptions().dtype(torch::kUInt8).device(device));
  {
    MTGR_NVTX_RANGE(2,
                    "MTGR/kernel/flashinfer_token_mask_base/mask_build");
    constexpr int threads = 256;
    dim3 grid(static_cast<unsigned int>(cache.batch_size),
              static_cast<unsigned int>(
                  (cache.max_mask_bytes_per_request + threads - 1) / threads));
    cudaStream_t stream = at::cuda::getCurrentCUDAStream().stream();
    mtgr_build_flashinfer_token_mask_kernel<<<grid, threads, 0, stream>>>(
        cache.packed_custom_mask.data_ptr<uint8_t>(),
        cache.mask_indptr_device.data_ptr<int32_t>(),
        segment_offsets_i32.data_ptr<int32_t>(),
        segment_rules_i32.data_ptr<int32_t>(),
        matched_prefix_lens_i32.data_ptr<int32_t>(),
        static_cast<int32_t>(cache.num_segments),
        static_cast<int32_t>(segment_offsets_i32.stride(0)),
        static_cast<int32_t>(cache.max_mask_bytes_per_request));
    C10_CUDA_KERNEL_LAUNCH_CHECK();
    if (xllm::mtgr_nvtx_enabled(2)) {
      C10_CUDA_CHECK(cudaStreamSynchronize(stream));
    }
  }

  {
    MTGR_NVTX_RANGE(2, "MTGR/kernel/flashinfer_token_mask_base/plan");
    auto plan = mtgr_build_token_mask_prefill_plan(device,
                                                   ...
                                                   cache.q_cu_seq_lens_host,
                                                   cache.kv_cu_seq_lens_host);
    cache.plan_uri = std::move(plan.uri);
    cache.plan_info = plan.plan_info;
  }
...
void mtgr_flashinfer_token_mask_attention_cuda(
    ...
    double sm_scale,
    torch::Tensor output_snd,
    const std::shared_ptr<xllm::layer::MTGRFlashinferTokenMaskCache>&
        token_mask_cache) {
  ...
  auto cache_ptr = token_mask_cache;
  if (!cache_ptr) {
    cache_ptr = std::make_shared<xllm::layer::MTGRFlashinferTokenMaskCache>();
  }
  auto& token_mask = *cache_ptr;
  if (!token_mask.initialized) {
    mtgr_build_token_mask_cache(token_mask,
                                device,
                                segment_offsets_i32,
                                segment_rules_i32,
                                q_seq_starts_i32,
    ...
    CHECK(mtgr_token_mask_cache_matches(token_mask,
                                        device,
                                        total_q,
                                        num_q_heads,
                                        num_kv_heads,
                                        ...
                                        key_snd.scalar_type(),
                                        output_snd.scalar_type()))
        << "MTGR FlashInfer token-mask cache shape/dtype/device mismatch";
  }

  torch::Tensor full_key_snd;
  torch::Tensor full_value_snd;
  const torch::Tensor* run_key_snd = &key_snd;
  const torch::Tensor* run_value_snd = &value_snd;
  const bool all_no_prefix = token_mask.total_kv_len == total_q;
  if (all_no_prefix) {
    MTGR_NVTX_RANGE(2,
                    "MTGR/kernel/flashinfer_token_mask_base/direct_live_kv");
    CHECK(key_snd.is_contiguous())
        << "MTGR no-prefix fast path expects contiguous live K";
    CHECK(value_snd.is_contiguous())
        << "MTGR no-prefix fast path expects contiguous live V";
    MTGR_TRACE(2) << "[KERNEL] flashinfer_token_mask_base direct_live_kv "
                  << "total_q=" << total_q
                  << " total_kv_len=" << token_mask.total_kv_len;
  } else {
    full_key_snd =
        torch::empty({token_mask.total_kv_len, num_kv_heads, head_dim},
                     key_snd.options());
    full_value_snd =
        torch::empty({token_mask.total_kv_len, num_kv_heads, head_dim},
                     value_snd.options());
    {
      ...
      const int64_t elems_per_request =
          token_mask.max_kv_len * num_kv_heads * head_dim;
      dim3 grid(static_cast<unsigned int>(token_mask.batch_size),
      ...
      mtgr_gather_full_kv_cache_bf16_kernel<<<grid, threads, 0, stream>>>(
          ...
          segment_offsets_i32.data_ptr<int32_t>(),
          token_mask.kv_cu_seq_lens_device.data_ptr<int32_t>(),
          block_table_i32.data_ptr<int32_t>(),
          static_cast<int32_t>(token_mask.num_segments),
          static_cast<int32_t>(segment_offsets_i32.stride(0)),
    ...
    run_key_snd = &full_key_snd;
    run_value_snd = &full_value_snd;
  }

  {
    ...
    cudaStream_t stream = at::cuda::getCurrentCUDAStream().stream();
    auto& ws = mtgr_flashinfer_workspace(device);
    get_function(token_mask.plan_uri, "ragged_run")(
        to_ffi_tensor(ws.float_workspace),
        to_ffi_tensor(ws.int_workspace),
        token_mask.plan_info,
        to_ffi_tensor(query_snd),
        to_ffi_tensor(*run_key_snd),
        to_ffi_tensor(*run_value_snd),
        to_ffi_tensor(token_mask.q_cu_seq_lens_device),
        to_ffi_tensor(token_mask.kv_cu_seq_lens_device),
        ...
        /*window_left=*/-1,
        support_pdl(),
        to_ffi_tensor(token_mask.packed_custom_mask),
        to_ffi_tensor(token_mask.mask_indptr_device),
...
void mtgr_kv_cache_writeback_cuda(
    ...
    const torch::Tensor& q_seq_starts_i32,
    const torch::Tensor& matched_prefix_lens_i32,
    const torch::Tensor& writeback_lens_i32,
    const torch::Tensor& block_table_i32,
    const torch::Tensor& key_cache,
    const torch::Tensor& value_cache,
    int64_t max_request_len) {
  ...
  if (writeback_lens_i32.defined() && writeback_lens_i32.numel() > 0) {
    CHECK_EQ(writeback_lens_i32.scalar_type(), torch::kInt32);
  }
  CHECK_EQ(block_table_i32.scalar_type(), torch::kInt32);
  ...
  if (writeback_lens_i32.defined() && writeback_lens_i32.numel() > 0) {
    CHECK_EQ(writeback_lens_i32.size(0), batch_size);
  }
  ...
  mtgr_kv_cache_writeback_bf16_kernel<<<grid, threads, 0, stream>>>(
      ...
      matched_prefix_lens_i32.data_ptr<int32_t>(),
      writeback_lens_i32.defined() && writeback_lens_i32.numel() > 0
          ? writeback_lens_i32.data_ptr<int32_t>()
          : nullptr,
      block_table_i32.data_ptr<int32_t>(),
