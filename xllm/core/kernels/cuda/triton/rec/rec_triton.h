...
class RecTritonKernel {
 public:
  ...
  void xattention(torch::Tensor q, 
 ...
 private:
  ...
  struct TensorDescriptorMetaData {
    explicit TensorDescriptorMetaData(const uint64_t* shape_ptr,
      ...
      strides = new uint64_t[input_tensor_rank - 1];
      ...
      memcpy(strides, strides_ptr, sizeof(uint64_t) * (input_tensor_rank - 1));
  ...
  // void combine_attention_kernel(torch::Tensor shared_out, 
  ...
  // CUdeviceptr tensor_desc_2_device(const TensorDescriptor& tensor_desc, 
