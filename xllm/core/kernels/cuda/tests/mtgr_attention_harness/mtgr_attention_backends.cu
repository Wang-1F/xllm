...
__global__ void build_mtgr_packed_mask_kernel(uint8_t* __restrict__ packed_mask,
                                              ...
                                              int64_t realtime_len,
                                              int64_t mask_kind,
                                              int64_t bytes_per_request,
                                              int64_t total_num_bytes) {
  const int64_t byte_idx =
      static_cast<int64_t>(blockIdx.x) * blockDim.x + threadIdx.x;
  if (byte_idx >= total_num_bytes) {
    return;
  }

  uint8_t packed = 0;
  const int64_t local_byte = byte_idx % bytes_per_request;
  const int64_t bit_base = local_byte * 8;
#pragma unroll
...
__global__ void build_mtgr_bsr_packed_mask_kernel(
    ...
    int32_t block_rows,
    int32_t request_block_rows,
    int32_t request_block_cols,
  ...
  const int32_t local_row_block =
      request_block_rows > 0 ? row_block % request_block_rows : row_block;
  ...
  const int32_t q_rows = min(
      ...
      static_cast<int32_t>(total_len -
                           static_cast<int64_t>(local_row_block) *
                               row_block_size));
...
#pragma unroll
  for (int bit = 0; bit < 8; ++bit) {
    ...
    const int64_t q =
        static_cast<int64_t>(local_row_block) * row_block_size + q_row;
    const int32_t global_col_block = bsr_indices[nz_begin + nz];
    const int32_t local_col_block =
        request_block_cols > 0 ? global_col_block % request_block_cols
                               : global_col_block;
    const int64_t k =
        static_cast<int64_t>(local_col_block) * col_block_size + col;
...
__global__ void mtgr_kv_writeback_bf16_kernel(
    ...
    const int32_t* __restrict__ writeback_lens,
  ...
  const int32_t policy_cacheable_end =
      segment_offsets[row * segment_offsets_stride + num_segments];
  const int32_t requested_writeback_end =
      writeback_lens == nullptr ? policy_cacheable_end : writeback_lens[row];
  const int32_t cacheable_end =
      requested_writeback_end < policy_cacheable_end ? requested_writeback_end
                                                     : policy_cacheable_end;
  if (cacheable_end <= matched) {
    return;
  }
...
[[maybe_unused]] torch::Tensor make_seq_indptr_host(int64_t seq_len) {
  return torch::tensor(
      {0, static_cast<int32_t>(seq_len)},
      torch::TensorOptions().dtype(torch::kInt32).device(torch::kCPU));
}

torch::Tensor make_uniform_seq_indptr_host(int64_t batch_size,
                                           int64_t seq_len) {
  CHECK_GT(batch_size, 0);
  CHECK_GT(seq_len, 0);
  std::vector<int32_t> values(static_cast<size_t>(batch_size + 1), 0);
  for (int64_t row = 0; row < batch_size; ++row) {
    values[static_cast<size_t>(row + 1)] =
        static_cast<int32_t>((row + 1) * seq_len);
  }
...
torch::Tensor make_uniform_mask_indptr(const torch::Device& device,
                                       int64_t batch_size,
                                       int64_t bytes_per_request) {
  CHECK_GT(batch_size, 0);
  CHECK_GT(bytes_per_request, 0);
  std::vector<int32_t> values(static_cast<size_t>(batch_size + 1), 0);
  for (int64_t row = 0; row < batch_size; ++row) {
    values[static_cast<size_t>(row + 1)] =
        static_cast<int32_t>((row + 1) * bytes_per_request);
  }
  return torch::tensor(values,
                       torch::TensorOptions()
                           .dtype(torch::kInt32)
...
struct BlockSparseInputs {
  int64_t request_count = 1;
  int64_t row_block_size = 0;
  int64_t col_block_size = 0;
  int64_t block_rows = 0;
  int64_t block_cols = 0;
  int64_t request_block_rows = 0;
  int64_t request_block_cols = 0;
  int64_t request_total_len = 0;
  int64_t request_padded_kv_len = 0;
...
void build_mtgr_packed_mask(torch::Tensor packed_mask,
  ...
  CHECK_GE(packed_mask.numel(), num_bytes * request_count);

  c10::cuda::CUDAGuard guard(packed_mask.device());
  constexpr int threads = 256;
  const int64_t total_num_bytes = num_bytes * request_count;
  const int blocks =
      static_cast<int>((total_num_bytes + threads - 1) / threads);
...
torch::Tensor build_one_stage_packed_mask_reusing_buffer(
  ...
  const int64_t bytes_per_request = packed_mask_num_bytes(seq_len, seq_len);
  auto packed_mask = reserve_reusable_packed_mask_buffer(
      device, bytes_per_request * request_count);
...
BlockSparseInputs build_mtgr_block_sparse_inputs(const torch::Device& device,
  ...
  inputs.request_block_rows = block_rows;
  inputs.request_block_cols = block_cols;
  inputs.request_total_len = total_len;
  inputs.request_padded_kv_len = padded_kv_len;
...
BlockSparseInputs build_mtgr_block_sparse_inputs_batched_same_shape(
    const torch::Device& device,
    ...
    int64_t col_block_size) {
  CHECK_GT(request_count, 0);
  auto single = build_mtgr_block_sparse_inputs(device,
                                               history_len,
                                               context_len,
                                               realtime_len,
                                               target_len,
  ...
  const int64_t total_len =
      history_len + context_len + realtime_len + target_len;
  const int64_t rows_per_request = single.block_rows;
  const int64_t cols_per_request = single.block_cols;
  const int64_t nnz_per_request = single.bsr_indices_host.numel();
  const int64_t mask_bytes_per_request = single.packed_mask.numel();
  const auto* base_qo = single.qo_indptr_host.data_ptr<int32_t>();
  const auto* base_bsr = single.bsr_indptr_host.data_ptr<int32_t>();
  const auto* base_indices = single.bsr_indices_host.data_ptr<int32_t>();
  const auto* base_kv_lens = single.kv_lens_host.data_ptr<int32_t>();
  auto base_mask_indptr_cpu =
      single.packed_mask_indptr.to(torch::kCPU).contiguous();
  const auto* base_mask_indptr = base_mask_indptr_cpu.data_ptr<int32_t>();
  ...
  std::vector<int32_t> kv_lens_host;
  std::vector<int32_t> packed_mask_indptr_host;
  qo_indptr_host.reserve(static_cast<size_t>(request_count * rows_per_request + 1));
  bsr_indptr_host.reserve(static_cast<size_t>(request_count * rows_per_request + 1));
  bsr_indices_host.reserve(static_cast<size_t>(request_count * nnz_per_request));
  kv_lens_host.reserve(static_cast<size_t>(request_count * rows_per_request));
  packed_mask_indptr_host.reserve(
      static_cast<size_t>(request_count * rows_per_request + 1));

  qo_indptr_host.push_back(0);
  bsr_indptr_host.push_back(0);
  packed_mask_indptr_host.push_back(0);
  for (int64_t request = 0; request < request_count; ++request) {
    const int32_t page_offset =
        static_cast<int32_t>(request * cols_per_request);
    for (int64_t row = 0; row < rows_per_request; ++row) {
      qo_indptr_host.push_back(
          static_cast<int32_t>(request * total_len + base_qo[row + 1]));
      const int32_t nz_begin = base_bsr[row];
      const int32_t nz_end = base_bsr[row + 1];
      for (int32_t nz = nz_begin; nz < nz_end; ++nz) {
        bsr_indices_host.push_back(base_indices[nz] + page_offset);
      }
      bsr_indptr_host.push_back(
          static_cast<int32_t>(bsr_indices_host.size()));
      kv_lens_host.push_back(base_kv_lens[row]);
      packed_mask_indptr_host.push_back(
          static_cast<int32_t>(packed_mask_indptr_host.back() +
                               (base_mask_indptr[row + 1] -
                                base_mask_indptr[row])));
    }
  }
  ...
  BlockSparseInputs inputs;
  inputs.request_count = request_count;
  inputs.row_block_size = row_block_size;
  inputs.col_block_size = col_block_size;
  inputs.block_rows = request_count * rows_per_request;
  inputs.block_cols = request_count * cols_per_request;
  inputs.request_block_rows = rows_per_request;
  inputs.request_block_cols = cols_per_request;
  inputs.request_total_len = total_len;
  inputs.request_padded_kv_len = single.padded_kv_len;
  inputs.padded_kv_len = request_count * single.padded_kv_len;
  inputs.max_packed_bytes_per_row = single.max_packed_bytes_per_row;
  inputs.qo_indptr_host = torch::tensor(qo_indptr_host, i32_cpu).contiguous();
  inputs.bsr_indptr_host =
      torch::tensor(bsr_indptr_host, i32_cpu).contiguous();
  inputs.bsr_indices_host =
      torch::tensor(bsr_indices_host, i32_cpu).contiguous();
  inputs.kv_lens_host = torch::tensor(kv_lens_host, i32_cpu).contiguous();
  inputs.qo_indptr = inputs.qo_indptr_host.to(device).contiguous();
  ...
  inputs.packed_mask_indptr =
      torch::tensor(packed_mask_indptr_host, i32_dev).contiguous();
  inputs.packed_mask =
      torch::empty({request_count * mask_bytes_per_request},
                   torch::TensorOptions().dtype(torch::kUInt8).device(device))
          .contiguous();
  return inputs;
}
...
void build_mtgr_block_sparse_packed_mask(BlockSparseInputs* inputs,
  ...
  build_mtgr_bsr_packed_mask_kernel<<<blocks, threads, 0, stream>>>(
      ...
      static_cast<int32_t>(inputs->request_block_rows),
      static_cast<int32_t>(inputs->request_block_cols),
...
pad_uniform_batched_kv_to_block_sparse_pages(
    const torch::Tensor& key_snd,
    const torch::Tensor& value_snd,
    int64_t request_count,
    int64_t request_total_len,
    int64_t request_padded_kv_len,
    int64_t col_block_size) {
  CHECK_EQ(key_snd.dim(), 3);
  CHECK_EQ(value_snd.sizes(), key_snd.sizes());
  CHECK_GT(request_count, 0);
  CHECK_GT(request_total_len, 0);
  CHECK_GE(request_padded_kv_len, request_total_len);
  CHECK_EQ(key_snd.size(0), request_count * request_total_len);
  CHECK_EQ(request_padded_kv_len % col_block_size, 0);
  if (request_count == 1) {
    return pad_kv_to_block_sparse_pages(
        key_snd, value_snd, request_padded_kv_len, col_block_size);
  }

  auto padded_key =
      torch::empty({request_count,
                    request_padded_kv_len,
                    key_snd.size(1),
                    key_snd.size(2)},
                   key_snd.options());
  auto padded_value =
      torch::empty({request_count,
                   ...
                   value_snd.options());
  auto key_bsnd =
      key_snd.view({request_count, request_total_len, key_snd.size(1), key_snd.size(2)});
  auto value_bsnd = value_snd.view(
      {request_count, request_total_len, value_snd.size(1), value_snd.size(2)});
  padded_key.narrow(1, 0, request_total_len).copy_(key_bsnd);
  padded_value.narrow(1, 0, request_total_len).copy_(value_bsnd);
  const int64_t tail = request_padded_kv_len - request_total_len;
  if (tail > 0) {
    padded_key.narrow(1, request_total_len, tail).zero_();
    padded_value.narrow(1, request_total_len, tail).zero_();
  }
  return {padded_key.view({request_count * request_padded_kv_len / col_block_size,
                           col_block_size,
                           key_snd.size(1),
                           key_snd.size(2)}),
          padded_value.view({request_count * request_padded_kv_len / col_block_size,
                             col_block_size,
                             value_snd.size(1),
                             value_snd.size(2)})};
}
...
torch::Tensor run_custom_mask_prefill(const torch::Tensor& query_snd,
  ...
  CHECK_GT(request_count, 0);
  CHECK_EQ(q_len % request_count, 0);
  CHECK_EQ(kv_len % request_count, 0);
  const int64_t q_len_per_request = q_len / request_count;
  const int64_t kv_len_per_request = kv_len / request_count;
  const int64_t num_heads = query_snd.size(1);
  const int64_t num_kv_heads = key_snd.size(1);
  const int64_t head_dim = query_snd.size(2);
  auto q_cu_host =
      make_uniform_seq_indptr_host(request_count, q_len_per_request);
  auto kv_cu_host =
      make_uniform_seq_indptr_host(request_count, kv_len_per_request);
  ...
  auto plan = build_custom_mask_prefill_plan(device,
                                             ...
                                             kv_cu_host);
  auto& ws = get_workspace_buffers(device);
  auto mask_indptr = request_count == 1
                         ? get_cached_mask_indptr(device, packed_mask.numel())
                         : make_uniform_mask_indptr(
                               device,
                               request_count,
                               packed_mask.numel() / request_count);
...
[[maybe_unused]] torch::Tensor run_mtgr_one_stage_full_base_nvtx_only(
    const torch::Tensor& query_snd,
    const torch::Tensor& key_snd,
    const torch::Tensor& value_snd,
    int64_t history_len,
  ...
  const int64_t seq_len =
      history_len + context_len + realtime_len + target_len;
  CHECK_EQ(query_snd.size(0) % seq_len, 0);
  CHECK_EQ(key_snd.size(0), query_snd.size(0));
  const int64_t request_count = query_snd.size(0) / seq_len;
  torch::Tensor packed_mask;
  if (emit_nvtx) {
  ...
  if (emit_nvtx) {
    ...
    output = run_custom_mask_prefill(
        query_snd, key_snd, value_snd, packed_mask, sm_scale, request_count);
    CHECK_EQ(cudaDeviceSynchronize(), cudaSuccess);
  } else {
    output = run_custom_mask_prefill(
        query_snd, key_snd, value_snd, packed_mask, sm_scale, request_count);
...
torch::Tensor run_mtgr_torch_native_full_base_nvtx_only(
    const torch::Tensor& query_snd,
    const torch::Tensor& key_snd,
    const torch::Tensor& value_snd,
    int64_t history_len,
    ...
    double sm_scale,
    bool emit_nvtx) {
  CHECK_EQ(heads, kv_heads)
      << "torch_native_base currently covers MHA/no-GQA";
  const int64_t seq_len =
      history_len + context_len + realtime_len + target_len;
  CHECK_EQ(query_snd.size(0) % seq_len, 0);
  CHECK_EQ(key_snd.size(0), query_snd.size(0));
  ...
  shape.history = history_len;
  shape.context = context_len;
  shape.realtime = realtime_len;
  shape.target = target_len;
  shape.matched_prefix = 0;

  torch::Tensor visible_mask;
  if (emit_nvtx) {
    xllm::MtgrNvtxRange range(
        2, "MTGR/harness/mtgr_attention/torch_native/mask_build");
    visible_mask =
        build_mtgr_torch_full_visible_mask(shape, query_snd.device()).contiguous();
    CHECK_EQ(cudaDeviceSynchronize(), cudaSuccess);
  } else {
    visible_mask =
        build_mtgr_torch_full_visible_mask(shape, query_snd.device()).contiguous();
  ...
  if (emit_nvtx) {
    xllm::MtgrNvtxRange range(
        2, "MTGR/harness/mtgr_attention/torch_native/attention");
    auto query = query_snd.view({request_count, seq_len, heads, head_dim})
                     .to(torch::kFloat32)
                     .contiguous();
    auto key = key_snd.view({request_count, seq_len, kv_heads, head_dim})
                   .to(torch::kFloat32)
                   .contiguous();
    auto value = value_snd.view({request_count, seq_len, kv_heads, head_dim})
                     .to(torch::kFloat32)
                     .contiguous();
    auto scores =
        torch::einsum("bqhd,bkhd->bqhk", {query, key}) *
        static_cast<float>(sm_scale);
    auto masked_scores =
        scores.masked_fill(visible_mask.logical_not().unsqueeze(0).unsqueeze(2),
                           -std::numeric_limits<float>::infinity());
    auto probs = torch::softmax(masked_scores, /*dim=*/-1);
    full_output = torch::einsum("bqhk,bkhd->bqhd", {probs, value})
                      .to(query_snd.scalar_type())
                      .contiguous();
    CHECK_EQ(cudaDeviceSynchronize(), cudaSuccess);
  } else {
    auto query = query_snd.view({request_count, seq_len, heads, head_dim})
  ...
  return full_output.view({request_count * seq_len, heads * head_dim})
      .contiguous();
}
...
torch::Tensor run_mtgr_block_sparse_full_base_nvtx_only(
  ...
  if (emit_nvtx) {
    ...
    inputs = build_mtgr_block_sparse_inputs_batched_same_shape(device,
                                                               request_count,
                                                               history_len,
                                                               context_len,
                                                               realtime_len,
  ...
  if (emit_nvtx) {
    ...
    std::tie(key_pages, value_pages) =
        pad_uniform_batched_kv_to_block_sparse_pages(key_snd,
                                                     value_snd,
                                                     request_count,
                                                     seq_len,
                                                     inputs.request_padded_kv_len,
                                                     inputs.col_block_size);
    CHECK_EQ(cudaDeviceSynchronize(), cudaSuccess);
  } else {
    std::tie(key_pages, value_pages) =
...
class FullFlashinferBaseBackend final : public IMTGRAttentionBackend {
 public:
  ...
  std::tuple<torch::Tensor, std::optional<torch::Tensor>> forward(
    ...
    auto query_snd =
        query.view({query.size(0), metadata_.num_heads, metadata_.head_dim})
            .contiguous();
    auto key_snd =
        key.view({key.size(0), metadata_.num_kv_heads, metadata_.head_dim})
            .contiguous();
    auto value_snd =
        value.view({value.size(0), metadata_.num_kv_heads, metadata_.head_dim})
            .contiguous();

    run_mtgr_kv_writeback_cuda(key_snd, value_snd, attn_metadata, kv_cache);
    CHECK_EQ(cudaDeviceSynchronize(), cudaSuccess);

    auto full_out = torch::empty_like(query_snd);
    xllm::kernel::cuda::mtgr_flashinfer_token_mask_attention_cuda(
        query_snd,
        key_snd,
        value_snd,
...
class TorchNativeBaseBackend final : public IMTGRAttentionBackend {
 public:
  explicit TorchNativeBaseBackend(MTGRAttentionHarnessMetadata metadata)
      : metadata_(std::move(metadata)) {}

  std::string name() const override { return "torch_native_base"; }

  std::string nvtx_root_name(
      const MTGRAttentionHarnessMetadata& metadata) const override {
    return std::string("MTGR/harness/mtgr_attention/torch_native/") +
           metadata.mode_name();
  }
  ...
  std::tuple<torch::Tensor, std::optional<torch::Tensor>> forward(
    ...
    const double scale =
        1.0 / std::sqrt(static_cast<double>(metadata_.head_dim));
    auto full_out = run_mtgr_torch_native_full_base_nvtx_only(
        query.view({query.size(0), metadata_.num_heads, metadata_.head_dim}),
        key.view({key.size(0), metadata_.num_kv_heads, metadata_.head_dim}),
        value.view({value.size(0), metadata_.num_kv_heads, metadata_.head_dim}),
        metadata_.history,
...
std::unique_ptr<IMTGRAttentionBackend> make_torch_native_base_backend(
    const MTGRAttentionHarnessMetadata& metadata) {
  return std::make_unique<TorchNativeBaseBackend>(metadata);
}
...
void run_mtgr_kv_writeback_cuda(
  ...
  mtgr_kv_writeback_bf16_kernel<<<grid, threads, 0, stream>>>(
      ...
      attn_metadata.mtgr_writeback_lens_i32.defined() &&
              attn_metadata.mtgr_writeback_lens_i32.numel() > 0
          ? attn_metadata.mtgr_writeback_lens_i32.data_ptr<int32_t>()
          : nullptr,
