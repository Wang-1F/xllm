...
std::vector<int64_t> env_int_list(const char* name,
                                  std::vector<int64_t> default_values) {
  const char* value = std::getenv(name);
  if (value == nullptr || *value == '\0') {
  ...
  return values.empty() ? default_values : values;
}

std::vector<std::string> env_string_list(
    const char* name,
...
MTGRAttentionTestShape to_test_shape(
...
std::unique_ptr<IMTGRAttentionBackend> make_backend_by_name(
    const std::string& name,
    const MTGRAttentionHarnessMetadata& metadata) {
  if (name == "full_flashinfer_base") {
    return make_full_flashinfer_base_backend(metadata);
  }
  if (name == "block_sparse_flashinfer_base") {
    return make_block_sparse_flashinfer_base_backend(metadata);
  }
  if (name == "torch_native_base") {
    return make_torch_native_base_backend(metadata);
  }
  if (name == "hopper_unified") {
    return make_hopper_unified_backend(metadata);
  }
  LOG(FATAL) << "unknown MTGR attention backend: " << name;
  return nullptr;
}

torch::Tensor run_backend_for_case(IMTGRAttentionBackend* backend,
                                   MTGRAttentionCaseData* data) {
  CHECK(backend != nullptr);
  if (backend->name() == "hopper_unified") {
    return run_hopper(backend, data);
  }
  return run_base(backend, data);
}
...
MTGRAttentionHarnessMetadata make_fixed_shape_metadata(int64_t pair_id,
                                                       int64_t total_len,
                                                       int64_t target_len,
                                                       int64_t heads,
                                                       int64_t head_dim) {
  CHECK_GT(total_len, target_len);
  constexpr int64_t kHistoryRatio = 1300;
  constexpr int64_t kContextRatio = 8;
  constexpr int64_t kRealtimeRatio = 100;
  constexpr int64_t kRatioSum =
      kHistoryRatio + kContextRatio + kRealtimeRatio;
  const int64_t prefix_len = total_len - target_len;
  const std::array<int64_t, 3> ratios = {
      kHistoryRatio, kContextRatio, kRealtimeRatio};
  std::array<int64_t, 3> lengths = {
      (prefix_len * ratios[0]) / kRatioSum,
      (prefix_len * ratios[1]) / kRatioSum,
      (prefix_len * ratios[2]) / kRatioSum};
  std::array<std::pair<int64_t, int>, 3> remainders = {
      std::make_pair((prefix_len * ratios[0]) % kRatioSum, 0),
      std::make_pair((prefix_len * ratios[1]) % kRatioSum, 1),
      std::make_pair((prefix_len * ratios[2]) % kRatioSum, 2)};
  ...
  int64_t assigned = lengths[0] + lengths[1] + lengths[2];
  for (int64_t i = 0; i < prefix_len - assigned; ++i) {
    ++lengths[static_cast<size_t>(remainders[static_cast<size_t>(i)].second)];
  }

  MTGRAttentionHarnessMetadata metadata;
  metadata.pair_id = pair_id;
  metadata.num_heads = heads;
  metadata.num_kv_heads = heads;
  metadata.head_dim = head_dim;
  metadata.history = lengths[0];
  metadata.context = lengths[1];
  metadata.realtime = lengths[2];
  metadata.target = target_len;
  metadata.matched_prefix = 0;
  metadata.block_size = kDefaultBlockSize;
  CHECK_EQ(metadata.total_len(), total_len);
  return metadata;
}

MTGRAttentionCaseData build_same_shape_batch_case_data(
    const MTGRAttentionHarnessMetadata& metadata,
    int64_t batch_size,
    const torch::Device& device) {
  ...
  const auto live_shape = to_test_shape(metadata);
  auto full_shape = live_shape;
  full_shape.matched_prefix = 0;
  std::vector<MTGRAttentionTestShape> live_shapes(
      static_cast<size_t>(batch_size), live_shape);
  std::vector<MTGRAttentionTestShape> full_shapes(
      static_cast<size_t>(batch_size), full_shape);
  ...
  full_queries.reserve(static_cast<size_t>(batch_size));
  full_keys.reserve(static_cast<size_t>(batch_size));
  full_values.reserve(static_cast<size_t>(batch_size));
  live_queries.reserve(static_cast<size_t>(batch_size));
  ...
  for (int64_t row = 0; row < batch_size; ++row) {
    (void)row;
    auto full_query =
        torch::randn({1, total, metadata.num_heads, metadata.head_dim},
                     options) *
        0.05;
    auto full_key =
    ...
    auto full_value =
        torch::randn({1, total, metadata.num_kv_heads, metadata.head_dim},
                     options) *
        0.05;
    auto query_snd = full_query.select(0, 0).contiguous();
    auto key_snd = full_key.select(0, 0).contiguous();
    auto value_snd = full_value.select(0, 0).contiguous();
    full_queries.push_back(query_snd);
    full_keys.push_back(full_key);
    full_values.push_back(full_value);
    live_queries.push_back(
        query_snd.narrow(0, metadata.matched_prefix, live).contiguous());
    live_keys.push_back(
        key_snd.narrow(0, metadata.matched_prefix, live).contiguous());
    live_values.push_back(
        value_snd.narrow(0, metadata.matched_prefix, live).contiguous());
  }

  auto full_setup = make_mtgr_partial_batch_setup(full_shapes,
                                                  full_keys,
                                                  full_values,
                                                  device,
                                                  torch::kBFloat16,
                                                  metadata.block_size);
  auto live_setup = make_mtgr_partial_batch_setup(live_shapes,
                                                  full_keys,
                                                  full_values,
                                                  device,
                                                  torch::kBFloat16,
  ...
  auto full_query_snd = torch::cat(full_queries, 0).contiguous();
  std::vector<torch::Tensor> full_key_snd;
  std::vector<torch::Tensor> full_value_snd;
  full_key_snd.reserve(full_keys.size());
  full_value_snd.reserve(full_values.size());
  for (size_t i = 0; i < full_keys.size(); ++i) {
    full_key_snd.push_back(full_keys[i].select(0, 0).contiguous());
    full_value_snd.push_back(full_values[i].select(0, 0).contiguous());
  }

  MTGRAttentionCaseData data;
  data.harness_metadata = metadata;
  data.full_input.metadata = std::move(full_setup.metadata);
  data.full_input.query =
      full_query_snd.view({batch_size * total,
                           metadata.num_heads * metadata.head_dim})
          .contiguous();
  data.full_input.key =
      torch::cat(full_key_snd, 0)
          .contiguous()
          .view({batch_size * total,
                 metadata.num_kv_heads * metadata.head_dim})
          .contiguous();
  data.full_input.value =
      torch::cat(full_value_snd, 0)
          .contiguous()
          .view({batch_size * total,
                 metadata.num_kv_heads * metadata.head_dim})
          .contiguous();
  data.full_input.kv_cache = std::move(full_setup.kv_cache);

  data.live_input.metadata = std::move(live_setup.metadata);
  data.live_input.query =
      torch::cat(live_queries, 0)
          .contiguous()
          .view({batch_size * live,
                 metadata.num_heads * metadata.head_dim})
          .contiguous();
  data.live_input.key =
      torch::cat(live_keys, 0)
          .contiguous()
          .view({batch_size * live,
                 metadata.num_kv_heads * metadata.head_dim})
          .contiguous();
  data.live_input.value =
      torch::cat(live_values, 0)
          .contiguous()
          .view({batch_size * live,
                 metadata.num_kv_heads * metadata.head_dim})
          .contiguous();
  data.live_input.kv_cache = std::move(live_setup.kv_cache);
  return data;
}
...
xllm::layer::AttentionMetadata make_dynamic_segment_metadata(
  ...
  metadata.mtgr_writeback_lens_i32 =
      torch::tensor({total}, i32_dev_opts).contiguous();
  (void)heads;
  (void)head_dim;
  return metadata;
}
...
TEST_F(MTGRAttentionHarnessTest, FixedShapeBatchPerfNvtxCsv) {
  torch::NoGradGuard no_grad;
  FLAGS_mtgr_nvtx_level = std::max(FLAGS_mtgr_nvtx_level, 2);
  const int warmup =
      std::max(0, env_int("XLLM_MTGR_HARNESS_FIXED_WARMUP", 1));
  const int repeat =
      std::max(1, env_int("XLLM_MTGR_HARNESS_FIXED_REPEAT", 1));
  const int target_len =
      std::max(1, env_int("XLLM_MTGR_HARNESS_FIXED_TARGET", 800));
  const int heads =
      std::max(1, env_int("XLLM_MTGR_HARNESS_FIXED_HEADS", 8));
  const int head_dim =
      std::max(1, env_int("XLLM_MTGR_HARNESS_FIXED_HEAD_DIM", 128));
  const auto total_lens = env_int_list(
      "XLLM_MTGR_HARNESS_FIXED_TOTAL_LENS", {2000, 3000, 4000, 5000});
  const auto batch_sizes = env_int_list(
      "XLLM_MTGR_HARNESS_FIXED_BATCH_SIZES", {1, 4, 8, 16});
  const auto backend_names = env_string_list(
      "XLLM_MTGR_HARNESS_FIXED_BACKENDS",
      {"full_flashinfer_base",
       "block_sparse_flashinfer_base",
       "torch_native_base",
       "hopper_unified"});
  const std::string labels_path = env_path(
      "XLLM_MTGR_HARNESS_FIXED_LABELS",
      "mtgr_attention_fixed_shape_batch_labels.csv");
  ...
  int64_t pair_id = 0;
  for (const int64_t total_len : total_lens) {
    CHECK_GT(total_len, target_len)
        << "fixed total_len must be larger than target_len";
    for (const int64_t batch_size : batch_sizes) {
      CHECK_GT(batch_size, 0);
      cases.push_back(
          {.metadata = make_fixed_shape_metadata(++pair_id,
                                                 total_len,
                                                 target_len,
                                                 heads,
                                                 head_dim),
  ...
  for (int i = 0; i < warmup; ++i) {
    for (const auto& perf_case : cases) {
      auto data = build_same_shape_batch_case_data(
          perf_case.metadata, perf_case.batch_size, device_);
      for (const auto& backend_name : backend_names) {
        auto backend = make_backend_by_name(backend_name, perf_case.metadata);
        CHECK(run_backend_for_case(backend.get(), &data).defined());
      }
    }
  }
  CHECK_EQ(cudaDeviceSynchronize(), cudaSuccess);
  ...
  for (const auto& perf_case : cases) {
    for (int r = 0; r < repeat; ++r) {
      ...
      for (const auto& backend_name : backend_names) {
        ...
        CHECK_EQ(cudaDeviceSynchronize(), cudaSuccess);
        ++idx;
        write_perf_label_row(labels,
                             idx,
                             backend->name(),
                             perf_case.metadata,
                             r + 1,
                             perf_case.batch_size);
        labels.flush();
        {
          const auto root = backend->nvtx_root_name(perf_case.metadata);
          xllm::MtgrNvtxRange range(1, root.c_str());
          CHECK(run_backend_for_case(backend.get(), &data).defined());
        }
        CHECK_EQ(cudaDeviceSynchronize(), cudaSuccess);
    ...
    std::fprintf(stderr,
                 "[MTGR][Harness][FixedShapeBatchPerfNvtxCsv] "
                 "pair_id=%lld batch=%lld total_q=%lld labels=%lld\n",
                 static_cast<long long>(perf_case.metadata.pair_id),
                 static_cast<long long>(perf_case.batch_size),
                 static_cast<long long>(perf_case.metadata.total_len()),
                 static_cast<long long>(idx));
    std::fflush(stderr);
  }

  std::fprintf(stderr,
               "[MTGR][Harness][FixedShapeBatchPerfNvtxCsv] done "
               "cases=%zu backends=%zu repeat=%d labels=%lld labels_path=%s\n",
               cases.size(),
               backend_names.size(),
               repeat,
               static_cast<long long>(idx),
...
TEST_F(MTGRAttentionHarnessTest, DynamicWritebackThenAttentionPrecision) {
  ...
  for (size_t case_idx = 0; case_idx < cases.size(); ++case_idx) {
    ...
    xllm::kernel::cuda::mtgr_kv_cache_writeback_cuda(
        ...
        write_metadata.mtgr_writeback_lens_i32,
...
TEST_F(MTGRAttentionHarnessTest, MixedBatchWritebackThenAttentionPrecision) {
  ...
  xllm::kernel::cuda::mtgr_kv_cache_writeback_cuda(
      ...
      write_setup.metadata.mtgr_writeback_lens_i32,
