...
xllm::layer::AttentionMetadata make_mtgr_attention_metadata(
  ...
  metadata.mtgr_writeback_lens_i32 =
      torch::tensor({shape.total_len()}, i32_dev_opts).contiguous();
  return metadata;
}
...
xllm::layer::AttentionMetadata make_mtgr_attention_metadata(
  ...
  std::vector<int32_t> writeback_lens_i32;
  ...
  writeback_lens_i32.reserve(shapes.size());
  ...
  for (const auto& shape : shapes) {
    ...
    writeback_lens_i32.push_back(static_cast<int32_t>(shape.total_len()));
  ...
  metadata.mtgr_writeback_lens_i32 =
      torch::tensor(writeback_lens_i32, i32_dev_opts).contiguous();
  return metadata;
}
...
void run_mtgr_kv_writeback_reference(
  ...
  torch::Tensor writeback_lens;
  if (attn_metadata.mtgr_writeback_lens_i32.defined() &&
      attn_metadata.mtgr_writeback_lens_i32.numel() > 0) {
    writeback_lens =
        attn_metadata.mtgr_writeback_lens_i32.to(torch::kCPU).contiguous();
  }
  ...
  const auto* writeback_ptr =
      writeback_lens.defined() ? writeback_lens.data_ptr<int32_t>() : nullptr;
  ...
  for (int64_t row = 0; row < batch_size; ++row) {
    ...
    const int64_t policy_cacheable_end = offsets_acc[row][num_segments];
    const int64_t requested_writeback_end =
        writeback_ptr == nullptr ? policy_cacheable_end : writeback_ptr[row];
    const int64_t cacheable_end =
        std::min(policy_cacheable_end, requested_writeback_end);
...
void write_perf_label_header(std::ostream& out) {
  out << "idx,backend,pair_id,mode,batch_size,heads,kv_heads,head_dim,history,context,"
...
void write_perf_label_row(std::ostream& out,
  ...
  out << idx << "," << backend << "," << metadata.pair_id << ","
      << metadata.mode_name() << "," << batch_size << ","
      << metadata.num_heads << "," << metadata.num_kv_heads << ","
      << metadata.head_dim << ","
