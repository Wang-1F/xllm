...
def load_cases_from_labels(path: Path, block_size: int) -> list[MTGRFlexCase]:
    ...
    with path.open("r", encoding="utf-8", newline="") as f:
        for row in csv.DictReader(f):
            batch_size = row.get("batch_size", "1") or "1"
            key = (row["pair_id"], row["mode"], batch_size)
            if key in seen:
                continue
            seen.add(key)
            cases.append(
...
def build_mtgr_direct_block_mask(
    ...
    def expand_batch(tensor: torch.Tensor) -> torch.Tensor:
        if case.batch_size == 1:
            return tensor
        return tensor.expand(case.batch_size, *tensor.shape[1:]).contiguous()

    kv_num_blocks_tensor = expand_batch(take_view(q_blocks, (1, 1, q_blocks)))
    kv_indices_tensor = expand_batch(
        take_view(q_blocks * kv_blocks, (1, 1, q_blocks, kv_blocks))
    )
    full_kv_num_blocks_tensor = expand_batch(take_view(q_blocks, (1, 1, q_blocks)))
    full_kv_indices_tensor = expand_batch(take_view(
        q_blocks * kv_blocks,
        (1, 1, q_blocks, kv_blocks),
    ))
    q_num_blocks_tensor = expand_batch(take_view(kv_blocks, (1, 1, kv_blocks)))
    q_indices_tensor = expand_batch(
        take_view(kv_blocks * q_blocks, (1, 1, kv_blocks, q_blocks))
    )
    full_q_num_blocks_tensor = expand_batch(take_view(kv_blocks, (1, 1, kv_blocks)))
    full_q_indices_tensor = expand_batch(take_view(
        kv_blocks * q_blocks,
...
def make_inputs(case: MTGRFlexCase, device: torch.device) -> tuple[torch.Tensor, ...]:
    ...
    shape = (case.batch_size, case.heads, case.total_len, case.head_dim)
