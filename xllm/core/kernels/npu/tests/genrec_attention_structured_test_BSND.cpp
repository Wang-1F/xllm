...
class GenRecDirectRunBSNDTest : public ::testing::Test {
 protected:
...
torch::Tensor reference_attention_scores_fp32(
  ...
  auto q = query_bnsd.to(torch::kCPU).to(torch::kFloat32);
  auto k = key_bnsd.to(torch::kCPU).to(torch::kFloat32);
  if (group > 1) {
...
torch::Tensor reference_scaled_dot_product_attention(
  ...
  auto v = value_bnsd.to(torch::kCPU).to(torch::kFloat32);
  if (group > 1) {
...
torch::Tensor BSNDToBNSD(const torch::Tensor& bsnd) {
  CHECK_EQ(bsnd.dim(), 4);
  return bsnd.permute({0, 2, 1, 3}).contiguous();
}

torch::Tensor BNSDToBSND(const torch::Tensor& bnsd) {
  CHECK_EQ(bnsd.dim(), 4);
  return bnsd.permute({0, 2, 1, 3}).contiguous();
}
...
// LSE merge（aclnnAttentionUpdate）。merge 侧须保证 localOut/lse 的 ND 展平顺序严格对齐。
void expect_genrec_v2_output_vs_reference_genrec_gold(
  ...
  print_fp32_pair_diff("RefFinalGoldDiff",
                       "final_real_time",
                       rt_final.to(torch::kCPU).to(torch::kFloat32),
                       ref.real_time.to(torch::kCPU).to(torch::kFloat32),
                       kRefAttentionRtol,
                       kRefAttentionAtol);
  print_fp32_pair_diff("RefFinalGoldDiff",
                       "final_target",
                       tgt_final.to(torch::kCPU).to(torch::kFloat32),
                       ref.target.to(torch::kCPU).to(torch::kFloat32),
                       kRefAttentionRtol,
                       kRefAttentionAtol);
...
torch::Tensor reference_lse_merge_flash_cpu(const std::vector<torch::Tensor>& outs,
  ...
  CHECK(k == 2 || k == 3);
  auto o1 = BSNDToBNSD(outs[0].to(torch::kCPU).to(torch::kFloat32));
  auto o2 = BSNDToBNSD(outs[1].to(torch::kCPU).to(torch::kFloat32));
  auto l1 = lses[0].to(torch::kCPU).to(torch::kFloat32);
  auto l2 = lses[1].to(torch::kCPU).to(torch::kFloat32);
  if (k == 2) {
    return BNSDToBSND(CombineFlashOutputs(o1, l1, o2, l2))
        .to(torch::kFloat32);
  }
  auto o3 = BSNDToBNSD(outs[2].to(torch::kCPU).to(torch::kFloat32));
  auto l3 = lses[2].to(torch::kCPU).to(torch::kFloat32);
  return BNSDToBSND(CombineFlashOutputsThree(o1, l1, o2, l2, o3, l3))
      .to(torch::kFloat32);
}
...
void print_genrec_merge_pipeline_diagnostics(
  ...
  auto o_rt1 = BSNDToBNSD(crt_out.slice(1, c, c + r).detach().cpu().to(torch::kFloat32));
  auto l_rt1 = crt_lse.slice(2, c, c + r).detach().cpu().to(torch::kFloat32);
  auto o_rt2 = BSNDToBNSD(rt_out.slice(1, 0, r).detach().cpu().to(torch::kFloat32));
  ...
  auto npu_rt =
      BSNDToBNSD(output.slice(0, batch_idx, batch_idx + 1)
                     .slice(1, h + c, h + c + r)
                     .detach()
                     .cpu()
                     .to(torch::kFloat32));
  ...
  auto o_t1 = BSNDToBNSD(
      crt_out.slice(1, c + r, c + r + t).detach().cpu().to(torch::kFloat32));
  auto m_t1 = crt_lse.slice(2, c + r, c + r + t).detach().cpu().to(torch::kFloat32);
  auto o_t2 = BSNDToBNSD(rt_out.slice(1, r, r + t).detach().cpu().to(torch::kFloat32));
  auto m_t2 = rt_lse.slice(2, r, r + t).detach().cpu().to(torch::kFloat32);
  auto o_t3 = BSNDToBNSD(target_out.detach().cpu().to(torch::kFloat32));
...
void expect_npu_lse_merge_matches_reference(const char* stage_name,
  ...
  auto got = npu_output_fp32.detach().to(torch::kCPU).to(torch::kFloat32);
...
void expect_npu_attention_matches_reference(const char* stage_name,
                                            aclrtStream stream,
                                            const torch::Tensor& npu_output_bsnd,
  ...
  auto ref = reference_scaled_dot_product_attention(
      BSNDToBNSD(query_bsnd),
      BSNDToBNSD(key_bsnd),
      BSNDToBNSD(value_bsnd),
      attn_mask_b11ss,
      scale);
  auto got = BSNDToBNSD(npu_output_bsnd.detach())
                 .to(torch::kCPU)
                 .to(torch::kFloat32);
  ref = ref.to(torch::kFloat32);
  ASSERT_EQ(ref.sizes(), got.sizes())
...
void expect_npu_lse_matches_reference(const char* stage_name,
  ...
  auto ref = reference_scaled_dot_product_attention_lse(
      BSNDToBNSD(query_bsnd), BSNDToBNSD(key_bsnd), attn_mask_b11ss, scale);
  auto got = npu_lse_bnsd.detach().to(torch::kCPU).to(torch::kFloat32);
  ref = ref.to(torch::kFloat32);
  ASSERT_EQ(ref.sizes(), got.sizes())
...
torch::Tensor combine_two_way_attention_update_like_benchmark(
  ...
  CHECK_EQ(out1.dim(), 4);
  CHECK_EQ(lse1.dim(), 4);
  CHECK_EQ(out1.scalar_type(), out2.scalar_type());

  const int64_t b = out1.size(0);
  const int64_t s = out1.size(1);
  const int64_t n = out1.size(2);
  const int64_t d = out1.size(3);
  const int64_t bsn = b * s * n;

  auto local_out1_flat =
      out1.contiguous().view({bsn, d}).to(torch::kFloat32);
  auto local_out2_flat =
      out2.contiguous().view({bsn, d}).to(torch::kFloat32);
  auto lse1_flat =
      lse1.permute({0, 2, 1, 3}).contiguous().view({bsn});
  auto lse2_flat =
      lse2.permute({0, 2, 1, 3}).contiguous().view({bsn});
  ...
  return out_flat.view({b, s, n, d}).to(out1.scalar_type());
}
...
torch::Tensor combine_three_way_attention_update_like_benchmark(
  ...
  CHECK_EQ(lse1.dim(), 4);
  CHECK_EQ(out1.scalar_type(), out2.scalar_type());
  CHECK_EQ(out1.scalar_type(), out3.scalar_type());
  ...
  auto local_out3_flat =
      out3.contiguous().view({bsn, d}).to(torch::kFloat32);
  auto lse1_flat =
      lse1.permute({0, 2, 1, 3}).contiguous().view({bsn});
  auto lse2_flat =
      lse2.permute({0, 2, 1, 3}).contiguous().view({bsn});
  auto lse3_flat =
      lse3.permute({0, 2, 1, 3}).contiguous().view({bsn});
  auto out_flat = torch::empty(
      {bsn, d},
      torch::TensorOptions().dtype(torch::kFloat32).device(out1.device()));
  ...
  for (int64_t i = 0; i < batch_size; ++i) {
    ...
    if (enable_reference_checks) {
      expect_npu_attention_matches_reference("history",
                                             stream,
                                             output.slice(0, i, i + 1).slice(1, 0, h),
                                             hist_query,
                                             hist_key,
                                             hist_value,
    ...
    // lse combine：按 localOut/lse 展平顺序一致（BSN）调用 aclnnAttentionUpdate。
    ...
    auto combined_target_output =
        output.slice(0, i, i + 1).slice(1, h + c + r, h + c + r + t);
    {
...
TEST_F(GenRecDirectRunBSNDTest, DirectRunGenRecV2SingleBatchBSND) {
  ...
  auto query = query_seq.unsqueeze(0);
  auto key = key_seq.unsqueeze(0);
  auto value = value_seq.unsqueeze(0);
  ...
  auto warmup_query = warmup_query_seq.unsqueeze(0);
  auto warmup_key = warmup_key_seq.unsqueeze(0);
  auto warmup_value = warmup_value_seq.unsqueeze(0);
  ...
  auto output_bnsd = BSNDToBNSD(output);
  auto query_bnsd = BSNDToBNSD(query);
  auto key_bnsd = BSNDToBNSD(key);
  auto value_bnsd = BSNDToBNSD(value);
  expect_genrec_v2_output_vs_reference_genrec_gold(
      "DirectRunGenRecV2SingleBatchBSND",
      output_bnsd,
      query_bnsd,
      key_bnsd,
      value_bnsd,
