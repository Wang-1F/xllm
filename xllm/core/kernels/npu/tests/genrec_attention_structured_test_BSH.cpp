...
class GenRecDirectRunBSHTest : public ::testing::Test {
 protected:
...
torch::Tensor BSHToBNSD(const torch::Tensor& bsh, int64_t num_heads) {
  CHECK_EQ(bsh.dim(), 3);
  CHECK_GT(num_heads, 0);
  auto c = bsh.contiguous();
  const int64_t h = c.size(2);
  CHECK_EQ(h % num_heads, 0) << "hidden must be divisible by num_heads";
  const int64_t head_dim = h / num_heads;
  return c.view({c.size(0), c.size(1), num_heads, head_dim})
      .permute({0, 2, 1, 3})
      .contiguous();
}

torch::Tensor BNSDToBSH(const torch::Tensor& bnsd) {
  CHECK_EQ(bnsd.dim(), 4);
  return bnsd.permute({0, 2, 1, 3})
      .contiguous()
      .view({bnsd.size(0), bnsd.size(2), bnsd.size(1) * bnsd.size(3)});
}
...
torch::Tensor reference_lse_merge_flash_cpu(const std::vector<torch::Tensor>& outs,
  ...
  auto to_bnsd = [&](const torch::Tensor& out, const torch::Tensor& lse) {
    if (out.dim() == 4) {
      return out.to(torch::kCPU).to(torch::kFloat32).contiguous();
    }
    CHECK_EQ(out.dim(), 3);
    return BSHToBNSD(out.to(torch::kCPU).to(torch::kFloat32), lse.size(1));
  };
  auto o1 = to_bnsd(outs[0], lses[0]);
  auto o2 = to_bnsd(outs[1], lses[1]);
  auto l1 = lses[0].to(torch::kCPU).to(torch::kFloat32);
  auto l2 = lses[1].to(torch::kCPU).to(torch::kFloat32);
  if (k == 2) {
    return BNSDToBSH(CombineFlashOutputs(o1, l1, o2, l2))
        .to(torch::kFloat32)
        .contiguous();
  }
  auto o3 = to_bnsd(outs[2], lses[2]);
  auto l3 = lses[2].to(torch::kCPU).to(torch::kFloat32);
  return BNSDToBSH(CombineFlashOutputsThree(o1, l1, o2, l2, o3, l3))
      .to(torch::kFloat32)
      .contiguous();
}
...
torch::Tensor combine_two_way_attention_update_like_benchmark(
  ...
  const int64_t s = out1.size(1);
  const int64_t n = lse1.size(1);
  const int64_t h = out1.size(2);
  CHECK_EQ(lse1.size(0), b);
  CHECK_EQ(lse1.size(2), s);
  CHECK_EQ(h % n, 0);
  const int64_t d = h / n;
  const int64_t bsh = b * n * s;

  auto out1_bnsd = BSHToBNSD(out1, n);
  auto out2_bnsd = BSHToBNSD(out2, n);
  auto local_out1_flat =
      out1_bnsd.contiguous().reshape({bsh, d}).to(torch::kFloat32);
  auto local_out2_flat =
      out2_bnsd.contiguous().reshape({bsh, d}).to(torch::kFloat32);
  auto lse1_flat =
  ...
  torch::Tensor out_bnsd = out_flat.clone().to(out1.scalar_type()).view({b, n, s, d});
  torch::Tensor out_ret = BNSDToBSH(out_bnsd);
...
torch::Tensor combine_three_way_attention_update_like_benchmark(
  ...
  auto out1_bnsd = BSHToBNSD(out1, n);
  auto out2_bnsd = BSHToBNSD(out2, n);
  auto out3_bnsd = BSHToBNSD(out3, n);
  auto local_out1_flat =
      out1_bnsd.contiguous().reshape({bsh, d}).to(torch::kFloat32);
  auto local_out2_flat =
      out2_bnsd.contiguous().reshape({bsh, d}).to(torch::kFloat32);
  auto local_out3_flat =
      out3_bnsd.contiguous().reshape({bsh, d}).to(torch::kFloat32);
  auto lse1_flat =
      lse1.squeeze(-1).to(torch::kFloat32).contiguous().reshape({bsh});
  auto lse2_flat =
      lse2.squeeze(-1).to(torch::kFloat32).contiguous().reshape({bsh});
...
void run_genrec_v2_single_batch(
  ...
  const int64_t num_heads = attn_metadata.num_heads;
  const int64_t num_kv_heads = attn_metadata.num_kv_heads;
  const int64_t head_dim = attn_metadata.head_dim;
  ...
  CHECK_EQ(query.size(1), value.size(1));
  CHECK_EQ(query.size(2), num_heads * head_dim);
  CHECK_EQ(key.size(2), num_kv_heads * head_dim);
  CHECK_EQ(value.size(2), num_kv_heads * head_dim);
  ...
  for (int64_t i = 0; i < batch_size; ++i) {
    ...
    // ---history 部分---
    auto hist_query = query.slice(0, i, i + 1).slice(1, 0, h).contiguous();
    auto hist_key = key.slice(0, i, i + 1).slice(1, 0, h).contiguous();
    auto hist_value = value.slice(0, i, i + 1).slice(1, 0, h).contiguous();
    ...
    if (enable_reference_checks) {
      auto hist_out_bnsd = BSHToBNSD(output.slice(0, i, i + 1).slice(1, 0, h), num_heads);
      auto hist_query_bnsd = BSHToBNSD(hist_query, num_heads);
      auto hist_key_bnsd = BSHToBNSD(hist_key, num_kv_heads);
      auto hist_value_bnsd = BSHToBNSD(hist_value, num_kv_heads);
    ...
    auto crt_query = query.slice(0, i, i + 1).slice(1, h, h + c + r + t).contiguous();
    auto crt_key = key.slice(0, i, i + 1).slice(1, 0, h + c).contiguous();
    auto crt_value = value.slice(0, i, i + 1).slice(1, 0, h + c).contiguous();
    ...
    if (enable_reference_checks) {
      auto crt_out_bnsd = BSHToBNSD(crt_out, num_heads);
      auto crt_query_bnsd = BSHToBNSD(crt_query, num_heads);
      auto crt_key_bnsd = BSHToBNSD(crt_key, num_kv_heads);
      auto crt_value_bnsd = BSHToBNSD(crt_value, num_kv_heads);
    ...
    auto rt_query = query.slice(0, i, i + 1).slice(1, h + c, h + c + r + t).contiguous();
    auto rt_key = key.slice(0, i, i + 1).slice(1, h + c, h + c + r).contiguous();
    auto rt_value = value.slice(0, i, i + 1).slice(1, h + c, h + c + r).contiguous();
    ...
    if (enable_reference_checks) {
      auto rt_out_bnsd = BSHToBNSD(rt_out, num_heads);
      auto rt_query_bnsd = BSHToBNSD(rt_query, num_heads);
      auto rt_key_bnsd = BSHToBNSD(rt_key, num_kv_heads);
      auto rt_value_bnsd = BSHToBNSD(rt_value, num_kv_heads);
    ...
    auto target_query = query.slice(0, i, i + 1).slice(1, h + c + r, h + c + r + t).contiguous();
    auto target_key = key.slice(0, i, i + 1).slice(1, h + c + r, h + c + r + t).contiguous();
    auto target_value = value.slice(0, i, i + 1).slice(1, h + c + r, h + c + r + t).contiguous();
    ...
    if (enable_reference_checks) {
      auto target_out_bnsd = BSHToBNSD(target_out, num_heads);
      auto target_query_bnsd = BSHToBNSD(target_query, num_heads);
      auto target_key_bnsd = BSHToBNSD(target_key, num_kv_heads);
      auto target_value_bnsd = BSHToBNSD(target_value, num_kv_heads);
    ...
    auto combined_real_time_output = output.slice(0, i, i + 1).slice(1, h + c, h + c + r);
    {
      auto rt_merged = combine_two_way_attention_update_like_benchmark(
          crt_out.slice(1, c, c + r),
    ...
    auto combined_target_output = output.slice(0, i, i + 1).slice(1, h + c + r, h + c + r + t);
    {
      auto tgt_merged = combine_three_way_attention_update_like_benchmark(
          crt_out.slice(1, c + r, c + r + t),
          crt_lse.slice(2, c + r, c + r + t),
          rt_out.slice(1, r, r + t),
    ...
    if (enable_reference_checks) {
      auto query_bnsd = BSHToBNSD(query, num_heads);
      auto key_bnsd = BSHToBNSD(key, num_kv_heads);
      auto value_bnsd = BSHToBNSD(value, num_kv_heads);
      auto output_bnsd = BSHToBNSD(output, num_heads);
      auto crt_out_bnsd = BSHToBNSD(crt_out, num_heads);
      auto rt_out_bnsd = BSHToBNSD(rt_out, num_heads);
      auto target_out_bnsd = BSHToBNSD(target_out, num_heads);
...
TEST_F(GenRecDirectRunBSHTest, DirectRunGenRecV2SingleBatchBSH) {
  ...
  attn_metadata.num_heads = kNumHeads;
  attn_metadata.num_kv_heads = kNumKvHeads;
  attn_metadata.head_dim = kHeadDim;
  ...
  auto output_bnsd = BSHToBNSD(output, kNumHeads);
  auto query_bnsd = BSHToBNSD(query, kNumHeads);
  auto key_bnsd = BSHToBNSD(key, kNumKvHeads);
  auto value_bnsd = BSHToBNSD(value, kNumKvHeads);
  expect_genrec_v2_output_vs_reference_genrec_gold(
      "DirectRunGenRecV2SingleBatchBSH",
      output_bnsd,
      query_bnsd,
      key_bnsd,
      value_bnsd,
  ...
  ASSERT_EQ(output.size(2), kNumHeads * kHeadDim);
