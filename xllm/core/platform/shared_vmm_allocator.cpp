...
SharedVMMAllocator::~SharedVMMAllocator() {
  ...
  VLOG(kGraphExecutorLogVerboseLevel)
      << "SharedVMMAllocator destroyed. High water mark: " << high_water_mark_
      << " bytes, total virtual spaces created: " << virtual_spaces_.size();
}
...
void SharedVMMAllocator::init(int32_t device_id, size_t reserve_size) {
  ...
  initialized_ = true;
  VLOG(kGraphExecutorLogVerboseLevel)
      << "SharedVMMAllocator initialized on device " << device_id_
      << ". Reserved virtual address space: " << reserved_size_ << " bytes ("
...
void SharedVMMAllocator::reset_allocation_pointer() {
  ...
  current_offset_ = 0;
  VLOG(kGraphExecutorLogVerboseLevel)
      << "SharedVMMAllocator::reset_allocation_pointer() called. "
...
void SharedVMMAllocator::switch_to_new_virtual_space() {
  ...
  for (size_t i = 0; i < handles_.size(); ++i) {
    VirPtr map_addr = new_space.base_ptr + i * granularity;
  ...
  VLOG(kGraphExecutorLogVerboseLevel)
      << "SharedVMMAllocator::switch_to_new_virtual_space() called. "
      << "Previous space: " << prev_space_index
      << ", previous offset: " << prev_offset << " bytes, "
      << "New space: " << current_space_index_ << ", base_ptr: 0x" << std::hex
      << new_space.base_ptr << std::dec
      << ", physical memory mapped: " << new_space.mapped_size << " bytes, "
...
void SharedVMMAllocator::map_physical_to_virtual(VirtualSpace& space,
  ...
  for (size_t i = start_handle_idx; i < handles_.size(); ++i) {
    VirPtr map_addr = space.base_ptr + i * granularity;
    vmm::map(map_addr, handles_[i], granularity, device_id_);
  }
...
void* SharedVMMAllocator::allocate(size_t size) {
  ...
  void* ptr = reinterpret_cast<void*>(current_space.base_ptr + current_offset_);
  current_offset_ += aligned_size;
  high_water_mark_ = std::max(high_water_mark_, current_offset_);

  VLOG(2) << "SharedVMMAllocator: Allocated " << size
...
void SharedVMMAllocator::deallocate(void* ptr) {
  ...
  VLOG(2) << "SharedVMMAllocator: deallocate called (no-op), ptr: " << ptr;
}
...
void SharedVMMAllocator::extend_mapping(size_t new_size) {
  ...
  while (mapped_size_ < new_size) {
    ...
    for (auto& space : virtual_spaces_) {
      VirPtr map_addr = space.base_ptr + mapped_size_;
  ...
  VLOG(kGraphExecutorLogVerboseLevel)
      << "SharedVMMAllocator::extend_mapping() extended from "
